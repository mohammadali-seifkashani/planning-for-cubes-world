...
def applicable_options(s, all_objects):
    result = []

    for obj in all_objects:
        pick_up_preconds = operators.pick_up_preconditions(s, obj)
        put_down_preconds = operators.put_down_preconditions(s, obj)
        if type(pick_up_preconds) == list:
            result.append((operators.pick_up, (s, obj), pick_up_preconds))
        if type(put_down_preconds) == list:
            result.append((operators.put_down, (s, obj), put_down_preconds))

        for obj2 in all_objects:
            if obj2 == obj:
                continue
            stack_preconds = operators.stack_preconditions(s, obj, obj2)
            unstack_preconds = operators.unstack_preconditions(s, obj, obj2)
            if type(stack_preconds) == list:
                result.append((operators.stack, (s, obj, obj2), stack_preconds))
            if type(unstack_preconds) == list:
                result.append((operators.unstack, (s, obj, obj2), unstack_preconds))

    return result


def create_all_p(s, all_objects):
    s = tuple(s)
    result = {}
    for obj1 in all_objects:
        result[(s, f'EMPTY({obj1})')] = np.inf
        for obj2 in all_objects:
            if obj2 == obj1:
                continue
            result[(s, f'HAVE-GRIPPER({obj1},{obj2})')] = np.inf
            result[(s, f'AT-ROOM({obj1},{obj2})')] = np.inf
            result[(s, f'HOLDING({obj1},{obj2})')] = np.inf
...
def initialize_delta_table(s, all_objects):
    s = tuple(s)
    result = create_all_p(s, all_objects)
    level_2_result = {}
...
def update_delta_2(s, delta_table, U, all_objects):
    result = delta_table.copy()
    options = applicable_options(U, all_objects)
...
def delta_2(s, g, all_objects):
    initial_data_table = initialize_delta_table(s, all_objects)
    U = s.copy()
    counter = 1
    while True:
        new_U, new_result = update_delta_2(s, initial_data_table, U, all_objects)
        counter += 1
...
def heuristic_forward_search(pi, s, g, all_objects, seen_states):
    s = sorted(s)
    stuple = tuple(s)
    ...
    # options = actions
    options = applicable_options(s, all_objects)
    ...
    for action_function, action_args, actions_preconds in options:
        ...
        delta_out = delta_2(effects, g, all_objects)
    ...
    while options:
        ...
        pi_prime = heuristic_forward_search(pi + [new_action], next_state, g, all_objects, seen_states)
...
def main():
    ...
    print(data)
    # pi = heuristic_forward_search(pi, data['initial'], data['goal'], data['objs'], set())
    # print(pi)
